...
void ScriptManager::createConfigTree() {
	...
	// TODO: SWITCH TO USING SETKEY()

	config_registry.try_emplace("ServerName", ConfigKey(ConfigKeyType::string, "ServerName", "BlackMoon"));
	config_registry.try_emplace("ServerPort", ConfigKey(ConfigKeyType::integer, "ServerPort", "7212"));
	config_registry.try_emplace("ScriptDirectory", ConfigKey(ConfigKeyType::string, "ScriptDirectory", "/data"));
	config_registry.try_emplace("BindOnlyGlobalAddress", ConfigKey(ConfigKeyType::boolean, "BindOnlyGlobalAddress", "true"));
	config_registry.try_emplace("LoginProtocolPort", ConfigKey(ConfigKeyType::integer, "LoginProtocolPort", "7213"));
	config_registry.try_emplace("GameProtocolPort", ConfigKey(ConfigKeyType::integer, "GameProtocolPort", "7214"));
	config_registry.try_emplace("StatusProtocolPort", ConfigKey(ConfigKeyType::integer, "StatusProtocolPort", "7215"));
	config_registry.try_emplace("MaxPlayers", ConfigKey(ConfigKeyType::integer, "MaxPlayers", "1000"));
	config_registry.try_emplace("OnePlayerOnlinePerAccount", ConfigKey(ConfigKeyType::boolean, "OnePlayerOnlinePerAccount", "true"));
	config_registry.try_emplace("AllowMultiLogin", ConfigKey(ConfigKeyType::boolean, "AllowMultiLogin", "true"));
	config_registry.try_emplace("MaxSameIPConnection", ConfigKey(ConfigKeyType::integer, "MaxSameIPConnection", "5"));
	config_registry.try_emplace("MaxConnection", ConfigKey(ConfigKeyType::integer, "MaxConnection", "5000"));
	config_registry.try_emplace("MaxConnectionPerIP", ConfigKey(ConfigKeyType::integer, "MaxConnectionPerIP", "8000"));
	config_registry.try_emplace("MaxConnectionPerAccount", ConfigKey(ConfigKeyType::integer, "MaxConnectionPerAccount", "5000"));
	config_registry.try_emplace("MaxConnectionPerCharacter", ConfigKey(ConfigKeyType::integer, "MaxConnectionPerCharacter", "5000"));
}
	...
	try
	{
		try
		{
			...
			{	// could possibly need further validation here
				std::cout << "Current ServerName: " << getKey("ServerName").value << std::endl;
				config_registry["ServerName"].value = ServerName;
				std::cout << "New ServerName: " << getKey("ServerName").value << std::endl;
			}
			else
			{
		...
		try
		{
			...
			if (ServerPort != 0)
			{
				std::cout << "Current ServerPort: " << getKey("ServerPort").value << std::endl;
				config_registry["ServerPort"].value = std::to_string(ServerPort);
				std::cout << "ServerPort: " << getKey("ServerPort").value << std::endl;
			}
			else
			{
